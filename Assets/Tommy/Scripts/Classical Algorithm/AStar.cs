...
namespace Tommy.Scripts.Classical_Algorithm
{
    ...
    public class AStar
    {
        ...
        {
            Vector2 carHead = new Vector2(Mathf.Cos(carAngle), Mathf.Sin(carAngle)) * .06f + carPosition;
            for (int i = 0; i < pixelData.Length; i+=3)
            {
                ...
                if (Vector2.Distance(pixelXY, carPosition) < .05f || Vector2.Distance(pixelXY, carHead) < .03f)
                {
                    pixelData[i + 0] = 204; // R
                    pixelData[i + 1] = 85; // G
                    pixelData[i + 2] = 0; // B
                }
                else if (Vector2.Distance(pixelXY, carPosition) < .065f || Vector2.Distance(pixelXY, carHead) < .045f)
                {
                    pixelData[i + 0] = 255; // R
                    pixelData[i + 1] = 255; // G
                    pixelData[i + 2] = 255; // B
